...
def move(command: MoveCommand, value: int, curr_pos: tuple[int, int]) -> tuple[int, int]:
    if command == MoveCommand.FORWARD:
        return curr_pos[0] + value, curr_pos[1]
    elif command ==  MoveCommand.DOWN:
        return curr_pos[0], curr_pos[1] + value
...
def calculate_result(entry: list[tuple[MoveCommand, int]]) -> tuple[int, int]:
    curr_pos = (0, 0)
    for i in entry:
